...
fn kangaroo(x1: i32, v1: i32, x2: i32, v2: i32) -> String {
    ...
    let (mut k_first, mut k_second) = if x1 < x2 {
        ((x1, v1), (x2, v2))
    } else {
        ((x2, v2), (x1, v1))
    };
    if k_first.1 <= k_second.1 {
        return "NO".to_string();
    }
    while k_first.0 < k_second.0 {
        k_first.0 += k_first.1;
        k_second.0 += k_second.1;
    }
    if k_first.0 == k_second.0 { "YES".to_string() } else { "NO".to_string() }
}
